...
describe.only("CommandLine", () => {
	...
	describe("Nullability", () => {
		...
		it.skip("doesn't write output to command line", async () => {
			 ...
			 *
			 * Although this can be implemented with a flag and a bunch of "if" statements, the cleanest way to implement
			 * it is with a small stub of the "process" global.
			 *
		...
		it.skip("defaults to no arguments", () => {
			 ...
			 *
			 * If a test calls commandLine.args(), it will get the command line args of the test runner—probably "quick."
			 * That's not useful, so the args() needs to be stubbed out too. Remove the ".skip" from this test and
			 * expand your process stub to support args() as well.
			 *
			 * Hints:
			 *
		...
		it.skip("allows arguments to be configured", () => {
			 ...
			 *
			 * When you're done with this challenge, if there's still time remaining, look for opportunities to refactor.
			 * Then take a look at the HttpServer infrastructure wrapper (in http/http_server.js). It's more complicated
			 * than CommandLine, but uses an embedded stub just like CommandLine. Look at the code and figure out how
			 * the stub works.
			 *
			 * If you still have additional time, look at the HttpClient wrapper (in http/http_client.js). It's the most
			 * complicated infrastructure wrapper, but implemented in the same way as CommandLine. Look at the
